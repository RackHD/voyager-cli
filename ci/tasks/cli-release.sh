...
pushd $GOPATH/src/github.com/RackHD/voyager-cli
  ...
  printf "Mission Control Center Release v${release_version}" > name
  printf "v${release_version}" > tag
  tar -czvf voyager-cli-v${release_version}.tar.gz ./bin/*
  zip -rq voyager-cli-v${release_version}.zip ./bin/*
  echo "New version released."
popd

cp -r $GOPATH/src/github.com/RackHD/voyager-cli release
